class ProtectedFieldSerializer:
    ...
    def make_protected_fields_write_only(self):
        if not hasattr(self.Meta, "protected_fields"):
            return
...
class WrapSerializerDataMixin:
    ...
    def get_unwrapped_fields(self):
        fields = getattr(self.Meta, "exclude_wrap_fields", [])
        return list(fields)

    def to_representation(self, instance):
        wrap_key = self.get_wrap_key()
        data = super().to_representation(instance)
        if type(wrap_key) != str:
            return data

        unwrapped_fields = self.get_unwrapped_fields()
        unwrapped_data = {
            field: data.pop(field) for field in unwrapped_fields if field in data
        }

        return {wrap_key: data, **unwrapped_data}
