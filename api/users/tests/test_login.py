...
from api.accounts.factories import UserFactory#, LoggedDeviceFactory
...
def test_login_with_two_factor_auth(self):
    self.user = UserFactory(has_phone_number=True, two_factor_authenticable=True)
    payload = {
        'email': self.user.email,
        'password': 'password',
    }
...
def test_login_with_trusted_device(self):
    ...
    response = self.client.post(self.url, payload, format='json', HTTP_X_DEVICE_ID=device.id)
